...
class ResNetSeries(nn.Module):
    def __init__(self, pretrained):
        ...
        if pretrained == 'supervised':
            ...
            checkpoint = torch.load('checkpoints/moco_v1_200ep_pretrain.pth.tar', map_location="cpu")
...
class Disentangler(nn.Module):
    def __init__(self, cin):
        ...
        self.attention_head = nn.Conv2d(cin, 1, kernel_size=3, padding=1, bias=False)
        self.bn_head = nn.BatchNorm2d(1)

    def forward(self, x, inference=False):
        N, C, H, W = x.size()
        if inference:
            ccam = self.bn_head(self.attention_head(x))
        else:
            ccam = torch.sigmoid(self.bn_head(self.attention_head(x)))
...
class Network(nn.Module):
    def __init__(self, pretrained='mocov2', cin=2048+1024):
        ...
        self.am_head = Disentangler(cin)
        self.from_scratch_layers = [self.am_head]

    def forward(self, x, inference=False):

        feats = self.backbone(x).detach()
        fg_feats, bg_feats, ccam = self.am_head(feats, inference=inference)
